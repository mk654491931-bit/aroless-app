  ...
  head: () => ({
    meta: [
      { title: "Kâr / ROI Takip Paneli — Aroless" },
      {
        name: "description",
        content:
          "Gerçek mağaza rakamlarını gir; ürün bazında net kâr, marj, ROAS, CAC ve başabaş ROAS'ı anlık hesapla.",
      },
      { property: "og:title", content: "Kâr / ROI Takip Paneli — Aroless" },
      {
        property: "og:description",
        content: "Gerçek satış verinle ürün bazlı net kâr ve ROAS takibi.",
      },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: withProGate(RoiPage),
});
...
const BLANK = {
  ...
  orders: 0,
  refunds: 0,
  expected_margin_pct: null as number | null,
  notes: "",
};
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: string }) {
  return (
    <div className="premium-card p-3 text-center">
      <div className="text-xl font-black" style={{ color: tone ?? "var(--accent-active)" }}>
        {value}
      </div>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-6">
        ...
        <Stat
          label="Net kâr"
          value={money(agg.profit, currency)}
          tone={agg.profit >= 0 ? "var(--profit)" : "#f87171"}
        />
        ...
        <Card className="premium-card mb-5 border-white/10">
          <CardContent className="grid gap-3 p-4 md:grid-cols-4">
            ...
            <label className="text-xs text-muted-foreground">
              Ürün maliyeti
              <Input {...num("cost_price")} />
            </label>
            <label className="text-xs text-muted-foreground">
              Satış fiyatı
              <Input {...num("sell_price")} />
            </label>
            <label className="text-xs text-muted-foreground">
              Kargo / birim
              <Input {...num("shipping_cost")} />
            </label>
            <label className="text-xs text-muted-foreground">
              Diğer / birim
              <Input {...num("other_cost")} />
            </label>
            <label className="text-xs text-muted-foreground">
              Reklam harcaması
              <Input {...num("ad_spend")} />
            ...
            <label className="text-xs text-muted-foreground">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    expected_margin_pct: e.target.value === "" ? null : Number(e.target.value),
                  }))
                }
              />
            </label>
            ...
            <Button
              onClick={() => save.mutate()}
              disabled={form.product_name.trim().length < 1 || save.isPending}
            >
            ...
            {agg.rows.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="py-8 text-center text-xs text-muted-foreground">
                  Henüz kayıt yok. “Ürün ekle” ile başla.
                </TableCell>
              </TableRow>
            )}
            {agg.rows.map(({ entry, stats }) => (
              <TableRow key={entry.id}>
                ...
                <TableCell className="text-right text-xs">
                  {money(stats.revenue, entry.currency)}
                </TableCell>
                <TableCell
                  className="text-right text-xs font-bold"
                  style={{ color: stats.netProfit >= 0 ? "var(--profit)" : "#f87171" }}
                >
                  {money(stats.netProfit, entry.currency)}
                </TableCell>
                <TableCell className="text-right text-xs">%{stats.marginPct.toFixed(1)}</TableCell>
                <TableCell className="text-right text-xs">
                  {(stats.roasPct / 100).toFixed(2)}x
                </TableCell>
                <TableCell className="text-right text-xs text-muted-foreground">
                  {stats.breakEvenRoas.toFixed(2)}x
                </TableCell>
                <TableCell className="text-right text-xs">
                ...
                <TableCell className="text-right text-xs">
                  {stats.vsExpectedPct == null ? (
                    ...
                    >
                      ...
                      {stats.vsExpectedPct >= 0 ? "+" : ""}
                      {stats.vsExpectedPct.toFixed(1)} p
                    </Badge>
                  )}
                </TableCell>
      ...
      {agg.rows.length > 0 && (
        <div className="premium-card mt-4 p-4 text-xs text-muted-foreground">
          <div className="mb-2 flex items-center gap-2 text-sm font-bold text-foreground">
            <Target size={14} /> Aksiyon önerileri
          </div>
          <ul className="list-inside list-disc space-y-1">
            {agg.rows
              ...
              .map((r) => (
                <li key={r.entry.id}>
                  <span className="text-foreground">{r.entry.product_name}</span>: zararda. Başabaş
                  için ROAS {r.stats.breakEvenRoas.toFixed(2)}x gerekiyor, şu an{" "}
                  {(r.stats.roasPct / 100).toFixed(2)}x.
                </li>
              ))}
            {agg.rows
              .filter((r) => r.stats.netProfit > 0)
              ...
              .map((r) => (
                <li key={r.entry.id}>
                  <span className="text-foreground">{r.entry.product_name}</span>: sipariş başına{" "}
                  {money(r.stats.profitPerOrder, r.entry.currency)} kâr. Bütçeyi kademeli artır.
                </li>
              ))}
          </ul>
        </div>
