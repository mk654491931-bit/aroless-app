...
import {
  ArrowLeft, Flame, Globe, Store, Wallet, TrendingUp, Truck, Users, Megaphone,
  AlertTriangle, Loader2, Package, Clock,
} from "lucide-react";
...
export const Route = createFileRoute("/hot/$id")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Bu ürünün hangi ülkede, hangi pazarda ve hangi bütçeyle satılması gerektiğini gösteren canlı saatlik pazar raporu." },
      { property: "og:title", content: "Canlı Ürün Fırsatı — Ülke, Pazar ve Bütçe Planı" },
      { property: "og:description", content: "Saatlik yenilenen gerçek pazar verisiyle ülke, kanal, maliyet ve bütçe planı." },
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-20 border-b border-border/60 bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-4xl items-center gap-3 px-4 py-3">
          ...
          <div className="ml-auto"><BrandLogo size="sm" /></div>
        </div>
      </header>
        ...
        {!isLoading && !p && (
          <div className="rounded-xl border border-border/60 bg-card/60 p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Liste her saat başı yenileniyor. Güncel fırsatlar için ana sayfadaki canlı akışa dönün.
            </p>
          </div>
        )}
          ...
          <article className="space-y-6">
              ...
              <div className="mt-3 overflow-hidden rounded-lg border border-border/60">
                <table className="w-full text-sm">
                  <tbody className="divide-y divide-border/60">
                    <tr><td className="px-3 py-2 text-muted-foreground">Tedarik maliyeti</td><td className="px-3 py-2 text-right font-medium text-foreground">{p.supplier_cost_usd}</td></tr>
                    <tr><td className="px-3 py-2 text-muted-foreground">Satış fiyatı</td><td className="px-3 py-2 text-right font-medium text-foreground">{p.retail_price_usd}</td></tr>
                    <tr><td className="px-3 py-2 text-muted-foreground">Rekabet</td><td className="px-3 py-2 text-right font-medium text-foreground">{p.competition}</td></tr>
                    <tr><td className="px-3 py-2 text-muted-foreground">Net marj</td><td className="px-3 py-2 text-right font-medium text-emerald-400">{p.margin_pct}%</td></tr>
                  </tbody>
                </table>
              </div>
            </section>
            ...
            <section className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-border/60 bg-card/60 p-5">
                <h2 className="flex items-center gap-2 text-sm font-bold text-foreground"><Users className="h-4 w-4" /> Hedef kitle</h2>
                <p className="mt-2 text-sm text-muted-foreground">{p.audience || "—"}</p>
              </div>
              <div className="rounded-xl border border-border/60 bg-card/60 p-5">
                <h2 className="flex items-center gap-2 text-sm font-bold text-foreground"><Megaphone className="h-4 w-4" /> Reklam açısı</h2>
            ...
            {p.risks.length > 0 && (
              <section className="rounded-xl border border-destructive/40 bg-destructive/5 p-5">
                <h2 className="flex items-center gap-2 text-sm font-bold text-foreground"><AlertTriangle className="h-4 w-4 text-destructive" /> Riskler</h2>
            ...
            {data && (
              <p className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                Son güncelleme {new Date(data.refreshed_at).toLocaleString()} · Sonraki yenileme {new Date(data.next_refresh_at).toLocaleTimeString()}
              </p>
            )}
          </article>
        )}
