...
import { Clapperboard, Loader2, Sparkles, Copy, Trash2, Wand2, Image as ImageIcon, Mail, FlaskConical } from "lucide-react";
...
import {
  generateCreativeKit, listCreativeAssets, deleteCreativeAsset, type CreativeAssetRow,
  ...
  return (
    <Tabs defaultValue="hooks" className="mt-4">
      <TabsList className="flex w-full flex-wrap justify-start gap-1 bg-white/5">
        <TabsTrigger value="hooks" className="text-xs"><Sparkles size={12} className="mr-1" /> Hook'lar</TabsTrigger>
        <TabsTrigger value="ugc" className="text-xs"><Clapperboard size={12} className="mr-1" /> UGC senaryo</TabsTrigger>
        <TabsTrigger value="ads" className="text-xs"><Wand2 size={12} className="mr-1" /> Reklam metni</TabsTrigger>
        <TabsTrigger value="visual" className="text-xs"><ImageIcon size={12} className="mr-1" /> Görsel promptları</TabsTrigger>
        <TabsTrigger value="tests" className="text-xs"><FlaskConical size={12} className="mr-1" /> A/B testler</TabsTrigger>
        <TabsTrigger value="crm" className="text-xs"><Mail size={12} className="mr-1" /> E-posta / SMS</TabsTrigger>
      </TabsList>

      <div className="premium-card mt-3 p-4 text-sm">
        <div className="mb-3 rounded-lg border border-white/10 bg-white/5 p-3">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Konumlandırma</div>
          ...
          {kit.hooks?.map((h, i) => (
            <div key={i} className="rounded-lg border border-white/10 bg-white/5 p-3">
              <div className="flex items-start justify-between gap-2">
                <Badge variant="outline" className="border-[var(--accent-active)]/30 text-[10px] text-[var(--accent-active)]">{h.angle}</Badge>
                <button onClick={() => copy(h.hook)} className="text-muted-foreground hover:text-foreground"><Copy size={13} /></button>
              </div>
        ...
        <TabsContent value="ugc">
          <div className="mb-2 flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-[10px]">{kit.ugc_script?.duration_seconds}s</Badge>
          </div>
          <div className="space-y-2">
            {kit.ugc_script?.scenes?.map((s, i) => (
              <div key={i} className="grid gap-2 rounded-lg border border-white/10 bg-white/5 p-3 md:grid-cols-[70px_1fr]">
                <div className="text-xs font-bold text-[var(--accent-active)]">{s.second}s</div>
                <div className="space-y-1 text-xs">
                  <div><span className="text-muted-foreground">Görsel: </span>{s.visual}</div>
                  <div><span className="text-muted-foreground">Ses: </span>{s.voiceover}</div>
                  <div><span className="text-muted-foreground">Yazı: </span>{s.text_overlay}</div>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-3 rounded-lg border border-[var(--profit)]/30 bg-[var(--profit)]/10 p-3 text-xs">CTA: {kit.ugc_script?.cta}</div>
        </TabsContent>
          ...
          {kit.ad_copies?.map((a, i) => (
            <div key={i} className="rounded-lg border border-white/10 bg-white/5 p-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => copy(`${a.headline}\n${a.primary}\n${a.description}\n${a.cta}`)} className="text-muted-foreground hover:text-foreground"><Copy size={13} /></button>
              </div>
              <p className="mt-2 font-semibold">{a.headline}</p>
              <p className="mt-1 whitespace-pre-line text-xs">{a.primary}</p>
              <p className="mt-1 text-xs text-muted-foreground">{a.description}</p>
              <div className="mt-2 inline-flex rounded-md bg-[var(--accent-active)]/15 px-2 py-1 text-[10px] text-[var(--accent-active)]">{a.cta}</div>
            </div>
          ))}
        </TabsContent>
          ...
          {kit.image_prompts?.map((p, i) => (
            <div key={i} className="rounded-lg border border-white/10 bg-white/5 p-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => copy(p.prompt)} className="text-muted-foreground hover:text-foreground"><Copy size={13} /></button>
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{p.prompt}</p>
            </div>
          ))}
          <div className="flex flex-wrap gap-1 pt-2">
            {kit.hashtags?.map((h, i) => <span key={i} className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[10px]">{h}</span>)}
          </div>
        </TabsContent>
...
function StudioPage() {
  ...
  const history = useQuery({ queryKey: ["creative-assets"], queryFn: () => listFn(), enabled: !!user });

  const gen = useMutation({
    mutationFn: () => genFn({ data: { product, platform, audience, price, tone, lang: getUiLang() } }),
    onSuccess: (row) => {
      setActive(row);
    ...
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <Card className="premium-card mb-5 border-white/10">
        <CardContent className="grid gap-3 p-4 md:grid-cols-5">
          <Input placeholder="Ürün adı" value={product} onChange={(e) => setProduct(e.target.value)} className="md:col-span-2" />
          ...
          <Select value={tone} onValueChange={setTone}>
            <SelectTrigger className="text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>{TONES.map((c) => <SelectItem key={c} value={c} className="text-xs">{c}</SelectItem>)}</SelectContent>
          </Select>
          <Button onClick={() => gen.mutate()} disabled={product.trim().length < 2 || gen.isPending}>
            {gen.isPending ? <Loader2 size={14} className="mr-1 animate-spin" /> : <Sparkles size={14} className="mr-1" />}
            Üret (1 kredi)
          </Button>
          <Input placeholder="Hedef kitle (opsiyonel)" value={audience} onChange={(e) => setAudience(e.target.value)} className="md:col-span-3" />
          <Input placeholder="Satış fiyatı (opsiyonel)" value={price} onChange={(e) => setPrice(e.target.value)} className="md:col-span-2" />
        </CardContent>
      </Card>
      ...
      {(history.data?.length ?? 0) > 0 && (
        <div className="mt-6">
          <div className="mb-2 text-xs uppercase tracking-wider text-muted-foreground">Geçmiş paketler</div>
          <div className="grid gap-2 md:grid-cols-3">
            {history.data!.map((row) => (
              <div key={row.id} className="premium-card flex items-center justify-between gap-2 p-3">
                <button className="min-w-0 flex-1 text-left" onClick={() => setActive(row)}>
                  ...
                  <div className="text-[10px] text-muted-foreground">{row.platform} · {new Date(row.created_at).toLocaleDateString()}</div>
                </button>
                <button
