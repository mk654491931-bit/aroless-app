  ...
  head: () => ({
    meta: [
      { title: "Rakip Analizi — Aroless" },
      {
        name: "description",
        content:
          "Bir ürün, ASIN veya mağaza URL'si gir; aktif satıcıları, fiyat trendlerini, müşteri şikâyetlerini ve AI karşı-strateji planını gör.",
      },
      { property: "og:title", content: "Rakip Analizi — Aroless" },
      {
        property: "og:description",
        content: "Aktif satıcı radarı, zayıflık analizi ve AI karşı-strateji playbook'u.",
      },
        ...
        >
          ...
          <div className="grid md:grid-cols-[1fr_auto] gap-3 items-end">
            ...
            <button
              ...
              className="rounded-lg bg-gradient-to-r from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] px-5 py-2.5 text-sm font-semibold text-white glow disabled:opacity-60 inline-flex items-center gap-2"
            >
              {mut.isPending ? (
                <>
                  <Loader2 size={16} className="animate-spin" /> Radar taranıyor…
                </>
              ) : (
                <>
                  <Swords size={16} /> Rakipleri Tara
                </>
              )}
            </button>
          </div>
          ...
          <>
            ...
            <section className="premium-card grain rounded-2xl p-5">
              <div className="flex items-center justify-between gap-3 mb-3">
                <h2 className="text-sm font-semibold flex items-center gap-2">
                  <Gauge size={14} /> Aktif Satıcı Radarı — <CountryFlag code={country} size={13} />{" "}
                  {c.name}
                </h2>
                <span className="text-xs text-muted-foreground">
                  Ortalama pazar fiyatı:{" "}
                  <span className="text-foreground font-semibold">
                    ${report.avg_price_usd.toFixed(2)}
                  </span>
                </span>
              </div>
              {report.trend_monthly && report.trend_monthly.length > 1 && (
                <div className="mb-4 rounded-lg border border-white/10 bg-white/[0.03] p-3">
                  <div className="flex items-center justify-between text-[11px] text-muted-foreground mb-1">
                    <span>
                      Google Trends — son 30 gün arama ilgisi
                      {report.trend_source === "estimated" ? " (tahmini)" : ""}
                    </span>
                    <span
                      className={
                        (report.trend_momentum_pct ?? 0) >= 0
                          ? "text-emerald-400 font-semibold"
                          : "text-rose-400 font-semibold"
                      }
                    >
                      {(report.trend_momentum_pct ?? 0) > 0 ? "+" : ""}
                      {report.trend_momentum_pct ?? 0}%
                    </span>
                  </div>
              ...
              {report.sellers.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  Bu sorgu için satıcı verisi bulunamadı — daha spesifik bir ürün adı ya da ASIN
                  dene.
                </p>
              ) : (
                    ...
                    <tbody>
                      {report.sellers.map((s, i) => (
                        <tr key={i} className="border-t border-white/10">
                            ...
                            {s.domain ? (
                              <span className="flex items-center gap-1.5">
                                ...
                                {typeof s.domain_rank === "number" && s.domain_rank > 0 && (
                                  <span className="rounded-full border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] text-emerald-300">
                                    DA {s.domain_rank.toFixed(1)}
                                  </span>
                                )}
                              </span>
                            ) : (
                          ...
                          <td className="py-2 pr-3 text-muted-foreground max-w-[220px]">
                            {s.margin_note}
                          </td>
                        </tr>
                      ))}
            ...
            <section className="premium-card grain rounded-2xl p-5">
              <h2 className="text-sm font-semibold flex items-center gap-2 mb-1">
                <MessageSquareWarning size={14} /> Zayıflık &amp; Şikâyet Analizi (Groq)
              </h2>
            ...
            {report.strategy && (
              <section className="premium-card grain rounded-2xl p-5">
                <h2 className="text-sm font-semibold flex items-center gap-2 mb-2">
                  <Lightbulb size={14} /> AI Karşı-Strateji Planı (Gemini)
                </h2>
                ...
                <div className="mt-3 rounded-lg border border-white/10 bg-white/[0.03] p-3 text-xs flex items-start gap-2">
                  <TrendingUp size={13} className="mt-0.5 text-emerald-400" />{" "}
                  <span>{report.strategy.price_advice}</span>
                </div>
                <ol className="mt-3 space-y-1.5 text-xs text-muted-foreground list-decimal list-inside">
                  {report.strategy.playbook.map((s, i) => (
                    <li key={i}>{s}</li>
                  ))}
                </ol>
                {report.strategy.ad_angle && (
                  <p className="mt-3 text-xs">
                    <span className="text-muted-foreground">Reklam açısı: </span>“
                    {report.strategy.ad_angle}”
                  </p>
                )}
              </section>
