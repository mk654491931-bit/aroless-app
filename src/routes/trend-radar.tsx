  ...
  head: () => ({
    meta: [
      { title: "Aroless Trend Radar — Çok Platformlu AI Trend Keşfi" },
      {
        name: "description",
        content:
          "Google, Amazon, TikTok ve Yandex açık verilerini kazıyan otomatik botlar + RSS akışları, Aroless Deep AI ile tek bir talep skorunda birleşiyor.",
      },
      { property: "og:title", content: "Aroless Trend Radar — Çok Platformlu AI Trend Keşfi" },
      {
        property: "og:description",
        content: "Kazınmış web verisi ve RSS akışlarından hibrit AI trend sentezi.",
      },
...
type Synthesis = {
  ...
  correlations: { theme: string; platforms: string[]; note: string }[];
};
...
function StatusDot({ ok }: { ok: boolean }) {
  return (
    <span
      className={`inline-block size-2 rounded-full ${ok ? "bg-emerald-400 animate-pulse" : "bg-rose-500"}`}
    />
  );
}
  ...
  return (
    <svg viewBox="0 0 60 60" className="size-14 -rotate-90">
      ...
      <text
        ...
        y="34"
        textAnchor="middle"
        className="rotate-90 origin-center fill-current text-[15px] font-black"
...
function TrendRadar() {
  ...
  const [active, setActive] = useState<TrendSource[]>([
    "Google",
    "Amazon",
    "TikTok",
    "Yandex",
  ...
  function scoreOf(t: ScrapedTrend): number {
    const hit = synth?.breakouts.find(
      (b) =>
    ...
    return Math.min(
      94,
      Math.round(t.metrics.growth_rate / 3 + Math.min(40, t.metrics.search_volume / 5000)),
    );
  }
  ...
  const distribution = useMemo(
    () =>
      SOURCES.map((s) => {
        ...
        const velocity = rows.length
          ? Math.round(rows.reduce((a, b) => a + b.metrics.growth_rate, 0) / rows.length)
          : 0;
    ...
    >
      ...
      <Card className="lg:col-span-2 border-white/10 bg-slate-950/60">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Kazıma botları, açık akışlar ve AI motoru tek panelden yönetilir.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            {SOURCES.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <b
                    className={st ? (ok ? "text-emerald-400" : "text-rose-400") : "text-slate-400"}
                  >
            ...
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1.5">
              <StatusDot ok /> <Brain size={12} /> AI Engine:{" "}
              <b className="text-emerald-400">Ready</b>
              {synth?.engines?.length ? (
                <span className="text-slate-400">({synth.engines.join(" + ")})</span>
              ) : null}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={runScrape} disabled={scraping} variant="secondary">
              {scraping ? <Loader2 className="animate-spin" size={15} /> : <Bot size={15} />}{" "}
              Trigger Automated Scraping Job
            </Button>
            <Button
              onClick={runAnalysis}
              disabled={analyzing}
      ...
      <Card className="lg:col-span-2 border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 via-slate-950/60 to-slate-950/60">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            {synth?.summary ||
              "Deep AI Analysis çalıştırıldığında en güçlü 3 breakout niş ürün, gerekçesi ve güven skoruyla burada belirir."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!synth ? (
                ...
                {synth.breakouts.slice(0, 3).map((b) => (
                  ...
                  >
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {b.sources.map((s) => (
                            <span
                              ...
                              style={{ background: `${srcColor(s)}22`, color: srcColor(s) }}
                            >
                              {s}
                            </span>
                          ))}
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      className="mt-2 h-7 px-2 text-xs text-cyan-300"
                    ...
                    {synth.correlations.map((c) => (
                      <li key={c.theme}>
                        ...
                        {c.platforms.map((p) => (
                          <span
                            key={p}
                            className="mx-0.5 rounded px-1 py-0.5 text-[10px]"
                            style={{ background: `${srcColor(p)}22`, color: srcColor(p) }}
                          >
                            {p}
                          </span>
                        ))}
      ...
      <Card className="border-white/10 bg-slate-950/60">
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-base">
            <TrendingUp size={16} className="text-emerald-400" /> Multi-Source Growth Comparison
          </CardTitle>
          <CardDescription>
            Kazınan/RSS metriklerinin sıralamaya göre büyüme eğrisi (%).
          </CardDescription>
        </CardHeader>
      ...
      <Card className="border-white/10 bg-slate-950/60">
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-base">
            <Globe size={16} className="text-violet-400" /> Platform Distribution Matrix
          </CardTitle>
      ...
      <Card className="lg:col-span-2 border-white/10 bg-slate-950/60">
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-base">
            <Search size={16} className="text-cyan-400" /> Trend Feed & AI Strategy
          </CardTitle>
          <CardDescription>
            {visible.length} sinyal — kaynak, hız, AI talep skoru ve ilk tespit zamanı.
          </CardDescription>
        </CardHeader>
                ...
                return (
                  <TableRow key={t.id}>
                    ...
                    <TableCell className="text-right tabular-nums">
                      {t.metrics.search_volume.toLocaleString()}
                    </TableCell>
                    <TableCell
                      className={`text-right tabular-nums ${t.metrics.growth_rate >= 100 ? "text-emerald-400" : t.metrics.growth_rate >= 40 ? "text-cyan-300" : "text-amber-300"}`}
                    >
                      +{t.metrics.growth_rate}%
                    </TableCell>
                    <TableCell className="text-right font-bold tabular-nums">{score}</TableCell>
                    <TableCell className="text-xs text-slate-400">
                      {new Date(t.scraped_at).toLocaleTimeString()}
                    </TableCell>
                    <TableCell className="text-right">
                      <Button
                        ...
                        onClick={() => openBrief(t.trend_name)}
                      >
      ...
      <Dialog open={!!brief} onOpenChange={(o) => !o && setBrief(null)}>
        <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-base">AI Ürün Brifingi — {brief?.trend}</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-base">Kaynak & AI Ayarları</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 text-sm">
            <div>
              <p className="mb-1.5 text-xs font-semibold text-slate-300">
                Açık RSS uç noktaları (satır başına bir URL)
              </p>
              <Textarea
                rows={4}
                value={rssText}
            ...
            <div>
              <p className="mb-1.5 text-xs font-semibold text-slate-300">
                Scraping webhook ingest (JSON dizi veya {"{items:[…]}"})
              </p>
              <Textarea
                rows={4}
              ...
              />
              <Button size="sm" className="mt-2" onClick={sendWebhook}>
                <Rss size={14} /> Payload gönder
              </Button>
            </div>
            <div>
              ...
              <Input
                readOnly
                value={`${typeof window !== "undefined" ? window.location.origin : ""}/api/public/trend-radar`}
              />
              <p className="mt-1 text-[11px] text-slate-500">
                POST {'{ action: "webhook", payload: […] }'}
              </p>
            </div>
            <p className="rounded-lg border border-white/10 bg-white/5 p-2 text-[11px] text-slate-400">
              AI anahtarları (Gemini ×3, Groq, OpenRouter, Lovable AI) sunucu tarafında güvenli
              şekilde saklanır — panelde girilmez.
            </p>
          </div>
        </DialogContent>
      </Dialog>
    ...
    <div className="space-y-4 text-sm">
      ...
      <Section title="✍️ Hazır Reklam Metinleri">
        {arr("ad_copy").map((a, i) => {
          ...
          return (
            <div key={i} className="rounded-lg border border-white/10 bg-white/5 p-2 text-xs">
              <Badge variant="outline" className="mb-1 border-cyan-500/40 text-cyan-300">
                {String(o["channel"] ?? "")}
              </Badge>
      ...
      {!!Object.keys(pricing).length && (
        <div className="grid grid-cols-3 gap-2 text-center text-xs">
          <Stat
            ...
            value={`$${Number(pricing["suggested_retail_usd"] ?? 0)}`}
            tone="text-emerald-400"
          />
          <Stat
            label="Maliyet"
            value={`$${Number(pricing["landed_cost_usd"] ?? 0)}`}
            tone="text-amber-300"
          />
          <Stat
            label="Marj"
            value={`%${Number(pricing["margin_pct"] ?? 0)}`}
            tone="text-cyan-300"
          />
        </div>
      )}
