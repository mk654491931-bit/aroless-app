...
import { ShieldCheck, Loader2, Search, Trash2, AlertTriangle, CheckCircle2, XCircle, Zap } from "lucide-react";
...
import { auditStore, listStoreAudits, deleteStoreAudit, type StoreAuditRow } from "@/lib/store-audit.functions";
  ...
  return (
    <div className="relative h-[104px] w-[104px] shrink-0">
      <svg width="104" height="104" viewBox="0 0 104 104" className="-rotate-90">
        <circle cx="52" cy="52" r={r} fill="none" stroke="currentColor" strokeWidth="9" className="text-white/10" />
    ...
    <div className="mt-5 space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="premium-card p-4">
          <div className="mb-2 flex items-center gap-2 text-sm font-bold text-red-400"><AlertTriangle size={14} /> Dönüşüm kırıcıları</div>
          <ul className="list-inside list-disc space-y-1 text-xs text-muted-foreground">
            {r.conversion_killers?.map((k, i) => <li key={i}>{k}</li>)}
          </ul>
        </div>
        <div className="premium-card p-4">
          <div className="mb-2 flex items-center gap-2 text-sm font-bold text-[var(--profit)]"><CheckCircle2 size={14} /> Güçlü yönler</div>
          <ul className="list-inside list-disc space-y-1 text-xs text-muted-foreground">
            {r.strengths?.map((k, i) => <li key={i}>{k}</li>)}
          </ul>
        </div>
      </div>
          ...
          {r.issues?.map((it, i) => (
            <div key={i} className="rounded-lg border border-white/10 bg-white/5 p-3">
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className={SEV[it.severity] ?? SEV['low']}>{it.severity}</Badge>
                <span className="text-[11px] uppercase tracking-wider text-muted-foreground">{it.area}</span>
              </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="grid grid-cols-2 gap-1.5 text-xs">
            {r.trust_signals?.map((s, i) => (
              <div key={i} className="flex items-center gap-1.5">
                {s.present ? <CheckCircle2 size={12} className="text-[var(--profit)]" /> : <XCircle size={12} className="text-red-400" />}
        ...
        <div className="premium-card p-4">
          ...
          <ul className="list-inside list-disc space-y-1 text-xs text-muted-foreground">
            {r.quick_wins?.map((k, i) => <li key={i}>{k}</li>)}
          </ul>
        </div>
      </div>
    </div>
...
function AuditPage() {
  ...
  const history = useQuery({ queryKey: ["store-audits"], queryFn: () => listFn(), enabled: !!user });

  const run = useMutation({
    mutationFn: () => auditFn({ data: { url: url.startsWith("http") ? url : `https://${url}`, lang: getUiLang() } }),
    onSuccess: (row) => {
    ...
    onError: (e: Error) =>
      toast.error(
        e.message === "NO_CREDITS" ? "Kredin bitti"
          : e.message === "FETCH_FAILED" ? "Sayfaya ulaşılamadı, adresi kontrol et"
          : e.message,
      ),
  });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <Card className="premium-card border-white/10">
        <CardContent className="flex flex-wrap gap-3 p-4">
          <Input
            ...
            onKeyDown={(e) => { if (e.key === "Enter" && url.length > 3) run.mutate(); }}
            className="min-w-[220px] flex-1"
          />
          <Button onClick={() => run.mutate()} disabled={url.length < 4 || run.isPending}>
            {run.isPending ? <Loader2 size={14} className="mr-1 animate-spin" /> : <Search size={14} className="mr-1" />}
            Denetle (1 kredi)
          </Button>
        </CardContent>
      </Card>
      ...
      {(history.data?.length ?? 0) > 0 && (
        <div className="mt-6">
          <div className="mb-2 text-xs uppercase tracking-wider text-muted-foreground">Geçmiş denetimler</div>
            ...
            {history.data!.map((row) => (
              <div key={row.id} className="premium-card flex items-center justify-between gap-2 p-3">
                <button className="min-w-0 flex-1 text-left" onClick={() => setActive(row)}>
                  ...
                  <div className="text-[10px] text-muted-foreground">Skor {row.health_score} · {new Date(row.created_at).toLocaleDateString()}</div>
                </button>
                <button
