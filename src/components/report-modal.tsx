      ...
      >
        ...
        <div className="mt-6 grid gap-5">
          ...
          <Section icon={TrendingUp} title={t("report.market_demand")}>
            <ul className="space-y-1.5 list-disc list-inside marker:text-[oklch(0.68_0.15_255)]">
          ...
          <Section icon={ShieldAlert} title={t("report.hooks")}>
            <ol className="space-y-1.5 list-decimal list-inside marker:text-[oklch(0.68_0.15_255)]">
              {(product.ad_angles || []).map((a, i) => (
                <li key={i}>{a}</li>
              ))}
            </ol>
        ...
        <div className="mt-6 flex flex-wrap gap-2 sticky bottom-0 -mx-6 md:-mx-8 px-6 md:px-8 pt-4 pb-1 border-t border-white/10 bg-gradient-to-t from-[oklch(0.17_0.03_255)] to-transparent">
          <button
            onClick={onPdf}
            className="rounded-lg bg-gradient-to-r from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] px-4 py-2 text-sm font-semibold text-white glow flex items-center gap-2"
          >
  ...
  return (
    <section>
      <h3 className="font-semibold mb-2 flex items-center gap-2 text-sm uppercase tracking-wider text-muted-foreground">
        <Icon size={14} className="text-[oklch(0.68_0.15_255)]" /> {title}
      </h3>
