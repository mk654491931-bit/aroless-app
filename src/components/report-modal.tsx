...
import { X, Download, Copy, Share2, Target, TrendingUp, DollarSign, Megaphone, Sparkles, ShieldAlert } from "lucide-react";
...
export function ReportModal({ product, onClose }: { product: WinningProduct | null; onClose: () => void }) {
  ...
  const onPdf = () => {
    ...
    line(`Supplier: ${product.supplier_price_usd}   Sell: ${product.selling_price_usd}   Margin: ${product.profit_margin_pct}%`);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="glass rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8 relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 end-4 p-1.5 rounded-lg hover:bg-white/10" aria-label={t("close")}><X size={18} /></button>
        <div className="flex items-start gap-3 pe-8">
          ...
          <div className="flex-1">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <span className={`text-xs px-2.5 py-1 rounded-full border font-semibold ${rec.cls}`}>{rec.emoji} {enriched.recommendation}</span>
              <span className="text-xs px-2.5 py-1 rounded-full border border-white/10 bg-white/5">AI {enriched.ai_score}/100</span>
              <span className="text-xs px-2.5 py-1 rounded-full border border-white/10 bg-white/5">Trend {enriched.trend_score}</span>
              <span className="text-xs px-2.5 py-1 rounded-full border border-white/10 bg-white/5">Confidence {enriched.confidence_score}</span>
            </div>
          </div>
        </div>

        <div className="mt-6 grid gap-5">
          <Section icon={Sparkles} title={t("report.executive_summary")}>{product.why_winning}</Section>
          <Section icon={Target} title={t("report.target_audience")}>{product.target_audience}</Section>
          <Section icon={TrendingUp} title={t("report.market_demand")}>
            <ul className="space-y-1.5 list-disc list-inside marker:text-[oklch(0.75_0.18_265)]">
              {(product.ad_angles || []).map((a, i) => <li key={i}>{a}</li>)}
            </ul>
          </Section>
          <Section icon={DollarSign} title={t("report.profitability")}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
              <Stat label={t("est_monthly_sales")} value={enriched.est_monthly_sales.toLocaleString()} />
              <Stat label={t("est_revenue")} value={formatCurrency(enriched.est_monthly_revenue_usd)} />
              <Stat label={t("net_profit")} value={formatCurrency(enriched.est_monthly_net_profit_usd)} />
              <Stat label={t("competition")} value={product.competition_level} />
            </div>
          </Section>
          <Section icon={Megaphone} title={t("report.marketing")}>{product.platform_strategy}</Section>
        ...
        <div className="mt-6 flex flex-wrap gap-2 sticky bottom-0 -mx-6 md:-mx-8 px-6 md:px-8 pt-4 pb-1 border-t border-white/10 bg-gradient-to-t from-[oklch(0.17_0.03_265)] to-transparent">
          <button onClick={onPdf} className="rounded-lg bg-gradient-to-r from-[oklch(0.68_0.20_265)] to-[oklch(0.66_0.24_305)] px-4 py-2 text-sm font-semibold text-white glow flex items-center gap-2">
            <Download size={14} /> {t("export_pdf")}
          </button>
          <button onClick={onCopy} className="rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 px-4 py-2 text-sm flex items-center gap-2">
            <Copy size={14} /> {t("copy")}
          </button>
          <button onClick={onShare} className="rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 px-4 py-2 text-sm flex items-center gap-2">
