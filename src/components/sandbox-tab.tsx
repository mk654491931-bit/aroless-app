      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {(
          ...
          <button
            key={id}
            onClick={() => setView(id)}
            className={`flex items-center gap-2 whitespace-nowrap rounded-xl px-4 py-2 text-sm font-medium transition ${view === id ? "bg-gradient-to-r from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] text-white glow" : "bg-white/5 hover:bg-white/10 text-muted-foreground"}`}
          >
            <Icon size={15} />
            {label}
          </button>
        ...
        <div className="fixed bottom-6 right-6 z-50 space-y-2">
          {celebrate.map((id) => {
            ...
            return (
              <div
                key={id}
                className="glass rounded-xl px-4 py-3 flex items-center gap-3 border border-[oklch(0.62_0.17_255)]/50 glow animate-in slide-in-from-right"
              >
  ...
  return (
    <div className="space-y-6">
      <div className="glass rounded-2xl p-6 md:p-8 relative overflow-hidden">
        <div className="absolute -top-24 -right-16 w-72 h-72 rounded-full bg-[oklch(0.62_0.17_255)]/20 blur-3xl" />
      ...
      <div className="glass rounded-2xl p-5 md:p-6 space-y-6">
            ...
            {SIM_PLATFORMS.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`text-left rounded-xl p-3 border transition ${on ? "border-[oklch(0.62_0.17_255)] bg-white/10 glow" : "border-white/10 bg-white/5 hover:bg-white/10"}`}
                >
          ...
          <div className="flex flex-wrap gap-2">
            {CAPITAL_OPTIONS.map((c) => (
              <button
                ...
                className={`rounded-full px-5 py-2 text-sm font-semibold transition ${capital === c ? "bg-gradient-to-r from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] text-white glow" : "bg-white/5 hover:bg-white/10"}`}
              >
        ...
        <Section title="3 · Product">
          <select
            ...
            className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2.5 text-sm outline-none focus:border-[oklch(0.62_0.17_255)]"
          >
            {catalog.map((c) => (
        ...
        <div className="flex items-center gap-2">
          ...
          {state.status === "running" ? (
            <button
              onClick={onNext}
              disabled={busy}
              className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold bg-gradient-to-r from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] text-white glow disabled:opacity-60"
            >
...
function CoachView({ state, advice }: { state: SandboxState; advice: CoachAdvice | null }) {
  return (
    <div className="grid lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2 glass rounded-2xl p-5">
        <h3 className="font-bold text-sm mb-3 flex items-center gap-2">
          <Brain size={15} className="text-[oklch(0.68_0.15_255)]" />
          AI coach & mentor
        </h3>
        {!advice ? (
              ...
              <ul className="space-y-2">
                {advice.actions.map((a, i) => (
                  <li key={i} className="flex gap-2 text-sm">
                    <ChevronRight
                      size={16}
                      className="text-[oklch(0.68_0.15_255)] shrink-0 mt-0.5"
                    />
                    {a}
                  </li>
                ))}
        ...
        <div className="grid sm:grid-cols-2 gap-2">
          {BADGES.map((b) => {
            ...
            return (
              <div
                key={b.id}
                className={`rounded-xl p-3 border ${on ? "border-[oklch(0.62_0.17_255)]/60 bg-white/10" : "border-white/10 bg-white/5 opacity-55"}`}
              >
        ...
        {lb.isLoading ? (
          ...
          <div className="space-y-1.5">
            {(lb.data?.rows ?? []).map((r, i) => (
              <div
                key={r.id}
                className={`flex items-center gap-3 rounded-lg px-3 py-2 text-xs ${r.is_me ? "bg-[oklch(0.62_0.17_255)]/20 border border-[oklch(0.62_0.17_255)]/40" : "bg-white/5"}`}
              >
