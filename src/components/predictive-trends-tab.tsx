...
import { CalendarClock, Flame, Loader2, Snowflake, Sparkles, TrendingDown, TrendingUp } from "lucide-react";
...
const VIEWS: { id: TrendView; label: string; icon: typeof Flame; hint: string }[] = [
  ...
  { id: "next", label: "🔮 Next Month's Winners", icon: CalendarClock, hint: "30-60 gün içinde zirveye çıkacak sezonluk fırsatlar" },
  { id: "season", label: "❄️/☀️ Seasonal Fast-Track", icon: Snowflake, hint: "Bu sezonun ülkeye özel en iyi performansçıları" },
];
    ...
    >
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2">
        <div className="min-w-0">
          ...
          <p className="truncate text-[11px] text-muted-foreground">{p.category} · {p.marketplace}</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[10px]">
        ...
        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 ${up ? "bg-emerald-400/15 text-emerald-300" : "bg-rose-400/15 text-rose-300"}`}>
          {up ? <TrendingUp size={10} /> : <TrendingDown size={10} />} {up ? "+" : ""}{p.momentum_pct}%
        </span>
        {p.spike_window && (
          <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5">{p.spike_window}</span>
        )}
        <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5">Rekabet: {p.competition}</span>
      ...
      <p className="text-[10px] font-semibold text-[oklch(0.78_0.16_290)]">Detaylı AI analizi için tıkla →</p>
    </article>
  );
}
    ...
    <section className="mx-auto max-w-6xl space-y-5">
      <header className="text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Google Trends verisi + Groq talep analizi · her saat başı yenilenir · pazar: <b>{country}</b>
        </p>
      </header>
      ...
      {!isLoading && !isFetching && (isError || items.length === 0) && (
        <p className="py-10 text-center text-sm text-muted-foreground">Şu anda trend verisi alınamadı, birazdan tekrar dene.</p>
      )}
      {!isFetching && items.length > 0 && (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {items.map((p) => <TrendCard key={p.id} p={p} onOpen={() => setSelected(p)} />)}
        </div>
      )}

      {selected && (
        <TrendAnalysisModal product={selected} country={country} onClose={() => setSelected(null)} />
      )}
    </section>
  );
}
