...
function TrendCard({ p, onOpen }: { p: TrendItem; onOpen: () => void }) {
  ...
  return (
    <article
      ...
      className="premium-card cursor-pointer rounded-2xl p-4 space-y-3 transition hover:border-white/20 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-[oklch(0.62_0.17_255)]"
    >
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2">
        ...
        <span className="shrink-0 rounded-full bg-gradient-to-r from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] px-2 py-0.5 text-[11px] font-semibold text-white">
          {p.score}
        </span>
      </div>
      ...
      <p className="text-[10px] font-semibold text-[oklch(0.72_0.13_255)]">
        ...
        {VIEWS.map((v) => {
          ...
          return (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-xs transition ${on ? "border-transparent bg-gradient-to-r from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] text-white glow" : "border-white/10 bg-white/5 text-muted-foreground hover:text-foreground"}`}
            >
              {v.label}
            </button>
          );
