...
export function DraggableCopilot({ context = "dashboard" }: { context?: string }) {
  ...
  return (
    <div className="fixed z-[60]" style={{ left: pos.x, top: pos.y }}>
      {open && (
        <div className="absolute bottom-16 right-0 w-[330px] max-w-[85vw] rounded-2xl border border-white/10 bg-[oklch(0.18_0.03_255)]/95 backdrop-blur-xl shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
            <span className="text-xs font-semibold flex items-center gap-1.5">
              <ArolessMark size={15} className="text-[oklch(0.78_0.16_255)]" /> Aroless Co-Pilot
            </span>
            <button
          ...
          <div className="max-h-64 overflow-y-auto p-3 space-y-2 text-xs">
            ...
            {msgs.map((m, i) => (
              <div
                key={i}
                className={`rounded-lg px-2.5 py-1.5 ${m.role === "user" ? "bg-white/10 ml-6" : "bg-[oklch(0.62_0.17_255)]/15 mr-6"}`}
              >
                {m.text}
              </div>
            ))}
          ...
          >
            <input
              ...
              maxLength={1000}
              className="flex-1 rounded-lg bg-white/5 border border-white/10 px-2.5 py-1.5 text-xs outline-none focus:border-[oklch(0.62_0.17_255)]"
            />
            <button
              type="submit"
              className="p-2 rounded-lg bg-gradient-to-r from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] text-white"
              aria-label="Gönder"
            >
              <Send size={13} />
            </button>
      ...
      <button
        ...
        className="h-14 w-14 rounded-full bg-gradient-to-br from-[oklch(0.62_0.17_255)] to-[oklch(0.52_0.15_262)] text-white shadow-xl glow grid place-items-center cursor-grab active:cursor-grabbing touch-none select-none"
