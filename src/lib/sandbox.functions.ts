...
export const startSimulation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ baseline: MarketBaseline; simCreditsRemaining: number }> => {
    ...
    try {
      ...
      baseline = {
        cvr_pct: num(parsed.cvr_pct, FALLBACK_BASELINE.cvr_pct, 0.1, 20),
        ctr_pct: num(parsed.ctr_pct, FALLBACK_BASELINE.ctr_pct, 0.1, 20),
        cpc_usd: num(parsed.cpc_usd, FALLBACK_BASELINE.cpc_usd, 0.05, 12),
        cac_usd: num(parsed.cac_usd, FALLBACK_BASELINE.cac_usd, 1, 400),
        avg_market_price_usd: num(parsed.avg_market_price_usd, data.price, 1, 5000),
        refund_rate_pct: num(parsed.refund_rate_pct, FALLBACK_BASELINE.refund_rate_pct, 0, 45),
        shipping_days: Math.round(num(parsed.shipping_days, FALLBACK_BASELINE.shipping_days, 1, 45)),
        organic_daily_visitors: num(parsed.organic_daily_visitors, FALLBACK_BASELINE.organic_daily_visitors, 0, 5000),
        seasonality: str(parsed.seasonality, FALLBACK_BASELINE.seasonality),
        risks: Array.isArray(parsed.risks) && parsed.risks.length ? parsed.risks.slice(0, 4).map(String) : FALLBACK_BASELINE.risks,
        benchmark_note: str(parsed.benchmark_note, FALLBACK_BASELINE.benchmark_note),
      };
    } catch {
      baseline = { ...FALLBACK_BASELINE, avg_market_price_usd: data.price };
    }
    return { baseline, simCreditsRemaining: Number(remaining ?? 0) };
  });

const CrisisInput = z.object({
  platform: z.string(), product: z.string(), day: z.number(), capital: z.number(),
  rating: z.number(), price: z.number(), adBudget: z.number(), recent: z.string().max(600),
});
...
export const getSimCrisis = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ crisis: Crisis | null }> => {
    try {
      ...
      if (!parsed.title || !Array.isArray(parsed.choices) || parsed.choices.length < 2) return { crisis: null };
      const scale = Math.max(1, data.capital / 2000);
      return {
        crisis: {
          title: String(parsed.title),
          body: String(parsed.body ?? ""),
          severity: (["low", "medium", "high"] as const).includes(parsed.severity as "low") ? parsed.severity! : "medium",
...
const ReviewInput = z.object({
  platform: z.string(), product: z.string(), price: z.number(), marketPrice: z.number(),
  shippingDays: z.number(), rating: z.number(), orders: z.number(),
});

export const getSimReviews = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) => ReviewInput.parse(i))
  .handler(async ({ data }): Promise<{ reviews: { stars: number; author: string; text: string }[] }> => {
    try {
      const text = await callGemini(reviewsPrompt(data), process.env.GEMINI_API_KEY, 1.0);
      const parsed = extractJson<{ reviews?: { stars?: number; author?: string; text?: string }[] }>(text, {});
      return {
        reviews: (parsed.reviews ?? []).slice(0, 3).map((r) => ({
...
const CoachInput = z.object({
  platform: z.string(), product: z.string(), day: z.number(),
  summary: z.string().max(800), state: z.string().max(800),
});
...
export const submitSimRun = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("sim_runs").insert({ ...data, user_id: context.userId });
...
export const getSimLeaderboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await (context.supabase.rpc as unknown as (
      fn: string,
    ) => Promise<{ data: SimLeaderboardRow[] | null; error: { message: string } | null }>)("get_sim_leaderboard");
    if (error) throw new Error(error.message);
...
export const getSimCredits = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data } = await context.supabase
      .from("profiles").select("sim_credits").eq("id", context.userId).maybeSingle();
