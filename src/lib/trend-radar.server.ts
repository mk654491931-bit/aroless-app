...
async function scrapeAmazon(region: string, category: string): Promise<ScrapedTrend[]> {
  ...
  if (!out.length)
    return await newsFallback("Amazon", "amazon best sellers trending product", region, category);
  return out;
}
...
async function scrapeTikTok(region: string, category: string): Promise<ScrapedTrend[]> {
  ...
  if (!list.length)
    return await newsFallback("TikTok", "tiktok viral product trend hashtag", cc, category);
  return list
    .slice(0, 20)
    .map((h, i) => {
    ...
    mk(
      ...
      category,
      {
        ...
        growth_rate: Math.min(
          300,
          Math.max(10, Math.round((r.stargazers_count / Math.max(1, r.forks_count || 1)) * 10)),
        ),
        rank: i + 1,
      },
      {
  ...
  return arr
    ...
    .map((raw, i) => {
      ...
      return mk(
        "RSS",
        "rss",
        name || `Webhook item ${i + 1}`,
        String(o["region"] ?? region),
...
export async function runScrapeJob(opts: {
  ...
  const want = new Set(
    opts.sources.length
      ? opts.sources
      : (["Google", "Amazon", "TikTok", "Yandex", "RSS"] as TrendSource[]),
  );
  ...
  const statuses: SourceStatus[] = settled.map((r, i) => {
    ...
    return {
      ...
      status: "error",
      items: 0,
      detail: String((r.reason as Error)?.message ?? "unreachable").slice(0, 80),
    };
  });
...
const MODE_BRIEF: Record<AiMode, string> = {
  ...
  strategy:
    "E-COMMERCE STRATEGY MODE: focus on how to sell it — offers, hooks, personas, creative angles, pricing.",
};
...
export async function runDeepAnalysis(
  ...
  if (!parsed.length) {
    return {
      mode,
      engines: [],
      summary: "AI motorlarına şu an ulaşılamadı — ham sinyaller aşağıda listeleniyor.",
      breakouts: [],
      noise_filtered: [],
      correlations: [],
      scores: {},
  ...
  for (const p of parsed) {
    for (const rawB of (p.data["breakouts"] as unknown[]).slice(0, 10)) {
      ...
      const arr = (k: string, n: number) =>
        Array.isArray(b[k]) ? (b[k] as unknown[]).slice(0, n).map(String) : [];
      ...
      if (cur) {
        ...
        cur.v.sources = Array.from(
          new Set([...cur.v.sources, ...(arr("sources", 5) as TrendSource[])]),
        );
      } else {
        byName.set(key, {
          scores: [score],
          v: {
            ...
            sentiment: (["positive", "neutral", "negative"].includes(String(b["sentiment"]))
              ? String(b["sentiment"])
              : "neutral") as AiTrendVerdict["sentiment"],
            hooks: arr("hooks", 3),
  ...
  const breakouts = Array.from(byName.values())
    .map(({ v, scores }) => {
      ...
      return {
        ...v,
        ai_demand_score: avg,
        confidence: Math.min(100, v.confidence + (scores.length - 1) * 8),
      };
    })
    .sort((a, b) => b.ai_demand_score - a.ai_demand_score)
    .slice(0, 8);

  const first = parsed[0]!.data;
  const noise = new Set<string>();
  parsed.forEach((p) =>
    (Array.isArray(p.data["noise_filtered"]) ? p.data["noise_filtered"] : [])
      .slice(0, 4)
      .forEach((n) => noise.add(String(n))),
  );

  const correlations = (Array.isArray(first["correlations"]) ? first["correlations"] : [])
    .slice(0, 4)
    .map((c) => {
      const o = (c ?? {}) as Record<string, unknown>;
      return {
        theme: String(o["theme"] ?? ""),
        platforms: (Array.isArray(o["platforms"]) ? o["platforms"] : []).map(
          String,
        ) as TrendSource[],
...
export async function runProductBrief(
  trend: string,
  region: string,
  category: string,
