...
- ✅ **Minification**: `minify: "esbuild"` — JS/SSR çıktısı küçültülür (build'i boşuna "unminified" bırakan eski ayar kaldırıldı)
- ✅ **CSS Minification**: `cssMinify` açık. CSS minifier `ES2020` hedefini okuyamadığı için `cssTarget` ayrıca verilir (eski halinde CSS hiç minify edilmiyordu)
- ✅ **Source Maps**: Yalnızca development'ta üretilir; production bundle'ına kaynak haritası eklenmez
- ✅ **Code Splitting**: `chunkSizeWarningLimit: 600`, `cssCodeSplit` ve rolldown'un otomatik bölmesi kullanılır (elle `manualChunks` listesi tutulmaz)
...
### 10. **Render Maliyeti: Paint & Compositor** (`src/lib/premium-mesh.ts`, `src/styles.css`, `src/components/premium-fx.tsx`)

Landing/auth sayfalarında kasmaya yol açan kalıplar tek tek ölçülüp düzeltildi. Ana kural: **her karede yeniden boyanan (paint) veya yeniden yerleşen (layout) bir animasyon yerine, sadece compositor'da çalışan `transform`/`opacity` animasyonu kullanmak.**

- ✅ **`background-position` animasyonları kaldırıldı**: `.ent-grid-bg` (6s'lik döngü, 52rem yükseklik) ve `.auth-grid` (tam ekran) artık bir karo fazlalıklı (overscan) katmanı `translate3d` ile kaydırır. Eski hali her karede tüm ekranı yeniden boyuyordu.
- ✅ **`left` animasyonları kaldırıldı**: `.amb-beam` ve CTA sheen (`cta-sheen`) artık `transform` ile akar; `left` her karede layout tetikliyordu.
- ✅ **Kalıcı `backdrop-blur` kaldırıldı**: sticky header'lar ve landing kartları sürekli animasyonlu ambient katmanların üzerinde duruyor; blur'lu arka plan her karede yeniden hesaplanıyordu. Yüzeyler biraz daha opak yapıldı, görünüm aynı kaldı.
- ✅ **Canvas mesh maliyeti sınırlandı** (`src/lib/premium-mesh.ts`): DPR 1.5 tavanı, alan başına düğüm sayısı, kare kapısı (30fps), kare mesafe kontrolü ve **opaklık kovaları** ile link başına `stroke()`/string üretimi yerine kova başına tek path.
- ✅ **Mesh yalnızca görünürken çalışır**: `IntersectionObserver` + `visibilitychange` ile ekran dışında/sekme arkada iken döngü durur.
- ✅ **İşaretçi (pointer) yazımları kareli**: `mousemove` başına transform yazmak yerine `requestAnimationFrame` ile tek yazım; touch ve `prefers-reduced-motion` cihazlarda parallax tamamen kapalı.

### 11. **Device Detection** (`src/lib/session-persistence.ts`)
...
const { displaySrc, isLoaded } = useLazyImageWithPlaceholder("high-quality.jpg", "placeholder.jpg");
```
...
- JavaScript (client + SSR) esbuild ile minify edilir
- CSS minify edilir (`cssTarget` ile birlikte)
- Kaynak haritası üretilmez
- Kod tree-shake edilir ve route bazında bölünür
- Build sonrası çıktı Vercel/Nitro `.vercel/output` altına yazılır
...
### Scroll/animasyon sırasında kasma

1. DevTools → Rendering → **Paint flashing** ile ekranın tamamının her karede yenilenip yenilenmediğine bak
2. DevTools → Performance → "Scrolling" kaydı al ve **Layout / Recalculate Style** bloklarını kontrol et
3. Bir öğe sticky + `backdrop-filter` kullanıyorsa veya sonsuz bir `background-position`/`left` animasyonu varsa, onu `transform` tabanlı bir alternatifle değiştir
...
- `vite.config.ts` - Build configuration (minify + cssMinify)
- `src/lib/premium-mesh.ts` - Canvas mesh maliyet kontrolü (saf, test edilmiş yardımcılar)
- `src/components/premium-fx.tsx` - Mesh/ambient render döngüleri
